...
struct bert_params
{
    int32_t n_threads;//  usually we use 6
    int32_t port; // server mode port to bind

    const char* model;// model path, "models/all-MiniLM-L6-v2/ggml-model-q4_0.bin"
    const char* prompt;// "test prompt"
};

BERT_API bool bert_params_parse(int argc, char **argv, struct bert_params &params);
...
BERT_API const char* bert_vocab_id_to_token(struct bert_ctx * ctx, bert_vocab_id id);
